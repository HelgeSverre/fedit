...
- `fedit commands [--json]`: Print the built-in command-palette reference (name, usage, summary) as a table, or as a JSON array with `--json`.
...
| `completionStyle`    | string   | `dock`                     | `dock` or `overlay`          | Where the completion popup shows: a list in the dock (above the status bar), or a floating popover anchored to the cursor.                                                                                                                                                                                                     |
...
| `autoReveal`         | bool     | `true`                     | —                            | Reveal (expand ancestors + select) a file in the sidebar when it is opened. Manual `:reveal` / reveal-in-sidebar always works regardless.                                                                                                                                                                                      |
| `ignoredNames`       | string[] | `[".git", "node_modules"]` | —                            | Basenames the file tree and watcher skip at any depth.                                                                                                                                                                                                                                                                         |
| `useGitignore`       | bool     | `true`                     | —                            | Honour every `.gitignore` under the workspace root (nearest file wins, `!` negation supported).                                                                                                                                                                                                                                |
| `languages`          | object   | `{}`                       | —                            | Extra tree-sitter grammars and query overrides, keyed by language id: `{ "vue": { "extensions": [".vue"], "library": "/path/libtree-sitter-vue.dylib", "queries": "/path/vue" } }`. `symbol` defaults to `tree_sitter_<name>`; an entry with only `queries` replaces a shipped language's `highlights.scm` / `injections.scm`. |
