...
- **CSV grids** (`:csv`) are display-only projections in `Model.CsvViews`: the buffer text (real separators, quoting) is untouched, so editing/undo/save are the plain text paths. `Csv.fs` owns the pure geometry — quote-aware `cellRanges`, separator detection, widths sampled from the first 1000 lines, and the `renderedX`/`textColAt` mapping shared by renderer, cursor, and mouse. Line 0 pins as a header row (viewport height shrinks by one in `updateActiveBuffer`); `ViewportLeft` runs in rendered (padded) space while a grid is active. Mutually exclusive with hex views. User-supplied paths (CLI arg, `:open`) go through `Paths.fromUser`, which maps Windows drive paths onto the WSL `/mnt` mount on Unix; `Paths.norm` never rewrites drives (LSP URIs round-trip them).
