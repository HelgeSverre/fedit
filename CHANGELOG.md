...
| Plugins | Hardened plugin name / path handling: `validateName` / `validateFileName` / `childPath` reject empty, dotted, rooted, or separator-bearing names so a plugin name can't escape the plugin root (path traversal). `LoadedPlugin` gains `Conflicts` so a session can surface keybinding / command clashes. The `Fedit.PluginApi.dll` contract now ships as a sidecar (in `libexec` on Homebrew, beside the binary on `just install`) so manifest-only plugins resolve it at build time. |
| Hex | Hex-edit mode for binary files. Files with a NUL byte in the first 8000 bytes open as a classic hex dump — offset column, hex byte cells (gap after each 8), ASCII column; 16 bytes per row, 8/4 on narrow terminals — and `:hex [on\|off\|toggle]` flips any buffer. Hex buffers hold the latin1 projection of the raw bytes (char offset = byte offset) so the piece table, undo, selection, and macros work unchanged in byte space; saves go through a byte-exact atomic write with no newline normalization. Nibble typing (ASCII pane overwrites whole bytes; `Tab` flips panes), byte-range selection/copy/cut with the spaced hex form on the clipboard, `/` search over hex byte sequences (`1a 2c 78`) or literal text, and `:replace <from> <to>` replace-all as one undo step (hex views read each argument as hex-else-literal, the search rule; text buffers replace literal exact-match text — the command works everywhere). A lossy hex→text flip (UTF-16/BOM transcode, invalid UTF-8, lone-CR normalization) marks the buffer modified and warns, so a save can't silently rewrite bytes from a buffer that read as clean. New `toggle-hex-view` action; status bar shows `HEX` plus byte offset/value; highlighting and LSP skip hex views. |
| Ayu | Added the `ayu` theme — Ayu Dark palette as the third full-surface theme (after `github-light` / `github-dark`): real backgrounds on every chrome region, `#E6B450` accent, and its own surface-tuned syntax colors. Spec mirror at `brand/themes/ayu.json`. Brings the bundled count to 13. |
| Completions | Expanded `fedit completions` from 3 shells to 9: zsh, bash, fish, pwsh, nushell, elvish, xonsh, yash, murex — plus OSH (Oils) via bash-script reuse. Generated scripts complete subcommands, flags, dynamic plugin names, and the `fedit <path>` positional. `--install` writes each shell's standard location and prints next-step instructions. Verified by a Docker smoke harness (`just test-completions`) loading all nine plus OSH, and parse checks in CI. |
| Format | Switched markdown formatting from prettier to oxfmt in the root `just format` / `just lint` recipes (fantomas still owns F#). The website keeps prettier + `prettier-plugin-astro` under `just website::format`. |
| Site | Upgraded the website from Astro 5 to Astro 6. |
| Actions | Seven append-only `PluginAction` cases: `OpenFilePreview`, `RevealPath`, `ReplaceRange(from, to_, text)`, `ClearSelection`, `DeleteSelection`, `SwitchBuffer(id)`, `NewBuffer(name, text)`. `WorkspaceView` grows `SelectedPath` (sidebar selection, absolute) and `Files` (root-relative sorted index); `Buffer.replaceRange` becomes public as the `ReplaceRange` primitive. `apiVersion` stays `"1"` under the now-documented append-only rule — new DU cases append at the end, new fields land only on host-constructed records. New `examples/jot` session scratchpad (`:jot`/`:jotdone`/`:jotgo`) exercises the set; `todo-list` reports into a `NewBuffer` scratch driven by `Workspace.Files`, `todo-next` continues into other buffers via `SwitchBuffer`, `journal` reveals the stamped file with `RevealPath`. `Fedit.PluginApi` 1.1.0. |
| Activation | Two append-only `PluginAction` cases: `SetBufferActivation commandName` registers a command to run when a line of the active buffer is activated (Enter or left-click), recorded against whatever buffer is active when the action runs; `OpenFileAt(path, position, preview)` opens a file and moves the cursor to a 1-based position, carrying the target through the async load so it survives the round-trip and applies in place when the file is already open. `todo-list` now opens a clickable TODO listing (`SetBufferActivation "todo-jump"` + `OpenFileAt`) instead of a static report. Unit tests for both cases plus a todo-list build-and-load end-to-end test. `Fedit.PluginApi` stays `apiVersion` "1" (append-only). |
...
| Plugins (out-of-process) | Plugins now load in a separate `Fedit.PluginHost` process instead of inside the editor. The editor holds only command specs and talks to the host over newline-delimited JSON-RPC (`PluginWire`/`PluginProtocol`, hand-rolled and reflection-free); the host (linking `Plugins.fs`) builds + loads plugins and runs their `Run` closures. The plugin API is unchanged (`apiVersion` "1"), so existing plugins keep working. Side effect: a slow plugin no longer freezes the UI (invocation moved out of `update` into a `PluginInvoke` effect). The host ships beside the editor in every release; the Homebrew formula installs it too. |
| AOT-ready | The editor is now NativeAOT-clean (purged ~140 `printf`/`%A` sites; FSharp.Core printf reflects via `MakeGenericMethod`, which AOT forbids). `just aot` builds a native bundle (~7 MB, ~10 ms first paint vs ~133 ms R2R) and `.github/workflows/aot-build.yml` produces per-RID artifacts on demand. AOT is opt-in; the default release stays self-contained R2R. The out-of-process plugin split is what makes AOT possible (AOT has no JIT to load plugin assemblies). |
| Cross-platform | Adopted a canonical forward-slash path model (`Paths.norm`/`Paths.parent`, applied at every OS boundary) so path comparisons, the file list, the plugin API, and the file picker behave identically on Windows/macOS/Linux. `.gitattributes` forces LF on checkout. Fixes 23 Windows-only test failures; Windows CI is green for the first time. |
| 1.5.1 — plugin host fix | Fixed: freshly-installed plugins (no pre-built `bin/`) silently failed to load. The single-file plugin host bundles `Fedit.PluginApi.dll`, so `Assembly.Location` was empty and the auto-generated plugin fsproj had no HintPath — `dotnet build` then failed. The host now resolves the dll from the sidecar beside it (shipped in every release). Validated by a per-RID AOT smoke (render + tree-sitter highlight + a full build→load→invoke plugin round-trip) now run across all five RIDs. |
| 1.6.0 — AOT archives shipped | Releases now ship opt-in `fedit-aot-<triple>` NativeAOT archives for all five RIDs (~7 MB, ~10 ms first paint), alongside the default self-contained R2R builds. NativeAOT can't cross-compile across OS (and Linux not across arch), so the release builds `linux-arm64` on a native ARM runner and cross-builds `osx-x64` from the arm64 macOS runner via the universal toolchain. Each shipped AOT bundle is smoke-tested (render + highlight + a full plugin round-trip) before upload. The default downloads and Homebrew stay R2R. |
| 1.7.0 — AOT becomes the default | NativeAOT is now the default release flavor: the `fedit-<triple>` archives, Homebrew, and the install scripts all ship the ~7 MB / ~10 ms native build. The self-contained R2R build moves to the opt-in `fedit-r2r-<triple>` archives as a fallback for any platform where a NativeAOT binary misbehaves. The Homebrew formula installs the whole bundle layout-agnostically (the AOT layout keeps tree-sitter natives loose in the root alongside `runtimes/` and the out-of-process plugin host). No functional change — plugins still run out-of-process, identically on both flavors. |
| 1.7.1 — AOT `--log` fix | Fixed: the `--log` dispatch trace crashed the default (NativeAOT) build. `renderMsg`/`renderEffect` fell through to `$"{msg}"`/`$"{effect}"`, which F# lowers to reflective structured printing — forbidden under AOT. Every `Msg`/`Effect` case is now rendered explicitly (only scalars, strings, lengths, and AOT-safe helpers reach an interpolation hole) and the wildcard is gone, so the match is exhaustive and a new case fails the build instead of silently reintroducing the reflective path. Verified against the AOT binary driven through a PTY with `--log`. Also: corrected the website/CLAUDE.md plugin-process-model docs (out-of-process), refreshed the manual AOT workflow's stale header, and added a `completions` smoke to the release AOT bundle (the Homebrew formula runs it at install time). |
| 1.7.2 — mouse drag selection | Fixed: dragging across a word left the character under the pointer unselected — drag "hello" ending on the `o` selected only "hell". The drag mapped the pointer's cell to its left-edge index, so the half-open selection dropped the trailing cell. The drag now extends the trailing edge one index past the hovered cell (only over a real character, so a drag past end-of-line does not swallow the newline), and the block cursor renders on the last selected glyph instead of the empty cell after it. Keyboard selection renders the same way. |
| 1.7.3 — sema grammar update | Updated the bundled tree-sitter-sema grammar to the canonical `sema-lisp/tree-sitter-sema` at v0.2.0 (was a personal fork). The grammar adds short-lambda `#(...)`, regex literals, reader `@`-deref, and shebang lines, plus string-literal fixes. Refreshed the bundled highlights query to match: `#(` joins the bracket set, and `regex` literals, `deref` operators, and shebang lines now highlight. |
| 1.8.0 — move lines | Added atomic line/block movement: `Alt+Up` / `Alt+Down` move the current line or every line containing selected text, clamp at buffer boundaries, preserve the selection, and create one undo entry. User keybindings can request larger counts with `move-lines-up:<count>` / `move-lines-down:<count>`. Plugins get matching append-only `MoveLinesUp count` / `MoveLinesDown count` actions in `Fedit.PluginApi` 1.3.0; manifest `apiVersion` remains `"1"`. |
| Quit + buffers | Unified the quit guard: every route (`Ctrl+Q`, `:quit`, rebound keys) shares the two-step dirty warning, which now names up to three dirty buffers; `quit force` skips it. New `close-buffer` (`Ctrl+W`, `:close [buffer]`) with the same two-step confirm, most-recently-used fallback activation, and a fresh scratch when the last buffer closes. Dirty buffers carry a ` [+]` marker in the `@` switcher and `:buffers` completions. |
| Focus + files | Clipboard paste (`Ctrl+V`) routes by focus like bracketed paste — the prompt takes the first line, the sidebar ignores it, the hidden buffer no longer mutates; Undo/Redo/SelectAll/Cut are inert while the prompt is focused. `Escape` clears the selection in editor focus. Opening a nonexistent path creates an empty buffer bound to it (file created on first save) instead of erroring; real I/O errors still notify. `:open` / `:writeas` / `:recent` completions reuse the file picker's case-insensitive substring matcher. |
| Notifications | Error notifications persist until `Escape` dismisses them (info/warning still clear on the next keypress), and the status segment is colored by severity via new `warningFg` / `errorFg` theme slots. A 100-entry ring buffer feeds `:messages` — a read-only picker of recent notifications with severity badges and the full text in the inspector. |
| Search flow | Gave in-buffer search accept/cancel semantics: `Enter` accepts — the prompt closes with the cursor at the current match and the query is remembered; `Escape` cancels — the cursor and viewport return to where the session began (captured on entering Search mode). `Up`/`Down` keep cycling matches inside the session. New `search-next` / `search-previous` actions (default `F3` / `Shift+F3`, editor context) repeat the remembered query from the cursor — synchronous and pure in `update`, wrapping like the prompt's cycling. Both paths share the `Buffer.findAllMatches` core (literal, case-insensitive), so prompt search and repeat search can never disagree. |
| Which-key + multi-click | While a multi-key sequence prefix is pending, the dock now lists every bound continuation for the active context (remaining stroke + action name), sized to content; it clears the instant the sequence fires, times out, or `Escape` cancels, and the abandon timeout grows from 1 s to 3 s so the panel is readable. Mouse presses carry a click count synthesized in the Runtime (the double-click window is wall-clock state, beside the prefix deadline): double-click selects the word under the cursor using the same boundary scans as the word motions, triple-click selects the whole line including its newline, and a fourth rapid click falls back to cursor placement. Dragging straight after a double-click keeps the word selection; extending it by words while dragging is deferred. |
| Macros v2 | Rewrote macros from chord capture to semantic recording, and made them persistent. Registers hold action / palette-command steps captured at the dispatch chokepoints — prompt outcomes, never prompt navigation — and replay through a step-fenced queue that waits for each step's async result (5 s timeout), with iteration counts, nested replay, exact cycle detection, and a depth cap of 8. New `insert-text` / `delete-backward` / `delete-forward` / `search-for` actions share one code path with typed keys; `Action.toSyntax` is the payload-preserving inverse of `Keymap.parseAction` (quoted free-text payloads with backslash escapes). Registers persist to `~/.config/fedit/macros` — line-oriented and human-editable (`REGISTER = step step …`, keybind action syntax plus `command:"…"` steps), written through on every recording commit or register clear, loaded at startup, and reloaded when the file is saved through fedit. The `:macros` picker gains `e` to open the file (created with a commented grammar header on first use). The last-macro marker stays per-session. Grammar reference in `docs/macros.md`. |
...
| LSP keys | Moved LSP navigation to JetBrains-style, terminal-safe primaries: `Ctrl+B` goto-definition, `Ctrl+Shift+B` find-references, `Ctrl+K` hover, `Ctrl+Alt+Left` jump-back. The old chords (`F12`, `Shift+F12`, `F1`, `Alt+-`) stay bound as secondaries — on a stock macOS setup the F-keys need `Fn`, `F1` is system help, and `Alt+-` types an en dash without option-as-meta. **Behavior change:** the tri-state sidebar toggle moved from `Ctrl+B` to `Ctrl+T` (T = tree) to free `Ctrl+B` for the definition jump. `Ctrl+Shift+B` needs the enhanced keyboard protocol (like the macro chords); legacy terminals send plain `Ctrl+B` and degrade to definition. `Ctrl+K` is a single binding, not a sequence prefix — user `ctrl+k …` sequences still win over it. |
| Ctrl+Click | `Ctrl+Click` in the editor text area jumps to the definition at the clicked position (the JetBrains/VSCode mouse gesture): the cursor lands on the clicked cell first, so the LSP request carries that position, then `goto-definition` dispatches through the macro-capture chokepoint. The press starts no selection drag; sidebar, status, and dock clicks keep their behavior with `Ctrl` held. |
| LSP quit | Quit no longer waits on a language server's exit etiquette: `shutdown` / `exit` are sent politely, the child gets 250 ms, then it is terminated (the spec requires servers to tolerate client death), and all clients dispose concurrently. Measured against `sema`, the old per-client 3 s `WaitForExit` made quit scale with the number of running servers. |
| 1.9.0 — LSP, macros v2, UX | Minor release: the LSP client (config-driven servers, JetBrains-style navigation, diagnostics, `:lsp` manager), the semantic macro system with the plain-text `~/.config/fedit/macros` file, the UX hardening pass (quit guard, close-buffer, focus routing, notifications, search flow, which-key, multi-click), and the fast-quit fix. Editor + plugin host bumped to 1.9.0; plugin API unchanged at 1.3.0. |
| Selection | Structural expand/shrink selection driven by the syntax tree fedit already parses — `Shift+Alt+Up` grows the selection to the smallest enclosing tree-sitter node, `Shift+Alt+Down` steps it back down (the Fleet / Eclipse / JetBrains-macOS / Helix arrow convention: up = out to the parent), with `Ctrl+Alt+W` as an expand secondary (NppTreeSitter / JetBrains lineage). The shrink chord contributed as `Ctrl+Alt+Shift+W` was rebound after a terminal survey: legacy encoding drops Shift on Ctrl+letter chords (it transmits identically to `Ctrl+Alt+W`), and a sticky-Alt slip is `Ctrl+Shift+W` — the close-tab chord in most Linux/Windows terminals. Modified arrows encode everywhere (`CSI 1;4A`) and every one-modifier slip is harmless; `Shift+Alt+Left/Right` stay reserved for future sibling-node navigation. The first expand parses off the UI thread (`ComputeSelectionLadder` effect → `SelectionLadderReady`) into an innermost→outermost range ladder cached on the model; subsequent steps are pure index moves, self-invalidating when the buffer is edited or the caret moves elsewhere. Results are double-stale-guarded: by `EditTick` (the `HighlightParsed` precedent) and by the requested selection echoed through the Msg — caret motion doesn't bump the tick, so without the echo a slow parse would select around wherever the caret is now. No LSP dependency — works for any loaded grammar; inert in hex views. Covered by ladder-algorithm tests (python + F#) and MVU wiring tests. |
| F# 0.3.3 | Bumped the `vendor/tree-sitter-fsharp` grammar submodule from 0.3.0 to 0.3.3 (latest ionide release) and re-synced the embedded `Resources/queries/fsharp/highlights.scm` to the 0.3.3 query (the prior copy was byte-identical to 0.3.0 upstream). Needs `just grammars` to rebuild the native lib before it takes effect at runtime. |
