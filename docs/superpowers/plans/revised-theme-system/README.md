...
The companion `prototype.html` is intentionally broader than a static theme swatch. It uses a flex app shell with a top control navbar, a padded full-size editor stage, and a searchable right-side color inspector. The prototype chrome uses OKLCH CSS tokens, a 12px-derived JetBrains Mono scale, antialiased body text, and an actual JetBrains Mono Nerd Font for icon preview. It exercises all bundled themes through the color panel, a realistic clickable project file tree, left/right sidebar docking, sidebar visibility, Nerd Font glyph toggling, editable command prompt text, preview modes with variant playback, completion selection, pending/invalid/empty states, right-aligned theme swatches, square color inputs, and editor-style code-pane scrolling. Treat it as a lightweight future sandbox for editor-surface design decisions.
