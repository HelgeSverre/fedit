...
The companion `prototype.html` is intentionally broader than a static theme swatch. It now exercises theme switching, collapsible file-tree rows, sidebar visibility, Nerd Font glyph toggling, command-bar open/closed state, command-bar mode cycling, completion selection, pending/invalid/empty command states, right-aligned theme swatches, and editor-style code-pane scrolling. Treat it as a lightweight future sandbox for editor-surface design decisions.
