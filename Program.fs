...
module PieceTable =
    ...
    let toString table =
        table.Pieces
        |> List.map (slice table)
        |> String.concat ""

    let length table =
        table.Pieces |> List.sumBy _.Length
        ...
        if String.IsNullOrEmpty text then
            ...
            let rec loop remaining acc pieces =
                ...
                | piece :: rest when remaining = 0 ->
                    List.rev acc @ (insertedPiece :: piece :: rest)
                | piece :: rest when remaining < piece.Length ->
                    ...
                    @ rest
                | piece :: rest ->
                    loop (remaining - piece.Length) (piece :: acc) rest
...
module Buffer =
    ...
    let line lineIndex buffer = rawLines buffer |> Array.tryItem lineIndex |> Option.defaultValue ""
    ...
    let private replaceRange startIndex count replacement buffer =
        ...
        let nextCursor = indexToPosition (startIndex + replacement.Length) { buffer with Document = inserted }
    ...
    let private withCursor position (buffer: BufferState) =
        { buffer with Cursor = clamp position buffer }

    let private withPreferredColumn column (buffer: BufferState) =
        { buffer with PreferredColumn = column }

    let moveLeft buffer = moveToIndex (max 0 (positionToIndex buffer.Cursor buffer - 1)) buffer
    let moveRight buffer = moveToIndex (positionToIndex buffer.Cursor buffer + 1) buffer

    let moveUp buffer =
        let targetColumn = buffer.PreferredColumn |> Option.defaultValue buffer.Cursor.Column

        buffer
        |> withCursor { Line = max 0 (buffer.Cursor.Line - 1); Column = targetColumn }
    ...
    let moveDown buffer =
        ...
        buffer
        |> withCursor { Line = min (lineCount buffer - 1) (buffer.Cursor.Line + 1); Column = targetColumn }
        |> withPreferredColumn (Some targetColumn)
    ...
    let moveEnd (buffer: BufferState) =
        { buffer with
            Cursor = { buffer.Cursor with Column = (line buffer.Cursor.Line buffer).Length }
            PreferredColumn = None }
    ...
    let movePageUp amount buffer =
        ...
        buffer
        |> withCursor { Line = max 0 (buffer.Cursor.Line - max 1 amount); Column = targetColumn }
        |> withPreferredColumn (Some targetColumn)
    ...
    let movePageDown amount buffer =
        ...
        buffer
        |> withCursor { Line = min (lineCount buffer - 1) (buffer.Cursor.Line + max 1 amount); Column = targetColumn }
        |> withPreferredColumn (Some targetColumn)

    let indent buffer =
        insertText tabText buffer

    let unindent buffer =
        let currentLine = line buffer.Cursor.Line buffer
        let removable = currentLine |> Seq.takeWhile ((=) ' ') |> Seq.length |> min tabText.Length

        if removable = 0 then
            buffer
        else
            let lineStart = positionToIndex { Line = buffer.Cursor.Line; Column = 0 } buffer
            let nextDocument = PieceTable.deleteRange lineStart removable buffer.Document
            let nextCursor =
                { Line = buffer.Cursor.Line
                  Column = if buffer.Cursor.Column <= removable then 0 else buffer.Cursor.Column - removable }
...
module Workspace =
    let excludedNames =
        set [ ".DS_Store"; ".git"; ".dotnet"; "bin"; "obj" ]
    ...
    let private sortChildren (nodes: FileNode list) =
        nodes |> List.sortBy (fun node -> (not node.IsDirectory, node.Name.ToLowerInvariant()))
    ...
    let rec private flatten selected expanded depth (node: FileNode) =
        ...
        if node.IsDirectory && Set.contains node.Path expanded then
            entry :: (node.Children |> sortChildren |> List.collect (flatten selected expanded (depth + 1)))
        else
            [ entry ]
    ...
    let private ensureSelected workspace =
        ...
        | _, first :: _ -> { workspace with SelectedPath = Some first.Path }
        | _ -> workspace

    let setTree (tree: FileNode) workspace =
        { workspace with
            Tree = Some tree
            Expanded = if tree.IsDirectory then Set.add tree.Path workspace.Expanded else workspace.Expanded }
        |> ensureSelected

    let selectPath path workspace =
        { workspace with SelectedPath = Some path } |> ensureSelected
    ...
    let moveHome workspace =
        ...
        | first :: _ -> { workspace with SelectedPath = Some first.Path }
        | [] -> workspace

    let moveEnd workspace =
        match visibleEntries workspace |> List.tryLast with
        | Some last -> { workspace with SelectedPath = Some last.Path }
        | None -> workspace

    let private findNodeByPath path workspace =
        let rec loop (node: FileNode) =
            if node.Path = path then Some node else node.Children |> List.tryPick loop

        workspace.Tree |> Option.bind loop

    let expandSelected workspace =
        match workspace.SelectedPath |> Option.bind (fun path -> findNodeByPath path workspace) with
        | Some node when node.IsDirectory -> { workspace with Expanded = Set.add node.Path workspace.Expanded }
        | _ -> workspace

    let collapseSelected workspace =
        match visibleEntries workspace |> List.tryFind (fun entry -> Some entry.Path = workspace.SelectedPath) with
        | Some entry when entry.IsDirectory && entry.IsExpanded ->
            { workspace with Expanded = Set.remove entry.Path workspace.Expanded }
        | Some entry ->
            let parent = Path.GetDirectoryName entry.Path
            if String.IsNullOrEmpty parent then workspace else selectPath parent workspace
        | None ->
            workspace
    ...
    let activateSelected workspace =
        ...
        | Some node ->
            workspace, SidebarOpenFile node.Path
        | None ->
            workspace, SidebarNoOp
    ...
    let metadata workspace =
        ...
        | Some node ->
            ...
            [ $"Path: {label}"
              $"Type: {nodeType}"
              (if node.IsDirectory then $"Children: {node.Children.Length}" else "Enter to open")
              "Ctrl+B tree"
              "Ctrl+E editor" ]
        | None ->
            [ "No file selected." ]
...
module Commands =
    ...
    let private simple command argument =
        if String.IsNullOrWhiteSpace argument then Ready command else Invalid "This command does not take arguments."

    let specs =
        [ { Name = "open"; Usage = "open <path>"; Summary = "Open a file from the workspace."; Constructor = fun argument -> if String.IsNullOrWhiteSpace argument then Pending "Path is required." else Ready (Open(argument.Trim())) }
          { Name = "write"; Usage = "write"; Summary = "Save the active buffer."; Constructor = simple Write }
          { Name = "writeas"; Usage = "writeas <path>"; Summary = "Save the active buffer to a new path."; Constructor = fun argument -> if String.IsNullOrWhiteSpace argument then Pending "Target path is required." else Ready (WriteAs(argument.Trim())) }
          { Name = "quit"; Usage = "quit"; Summary = "Exit fedit."; Constructor = simple Quit }
          { Name = "sidebar"; Usage = "sidebar"; Summary = "Toggle the sidebar."; Constructor = simple ToggleSidebar }
          { Name = "tree"; Usage = "tree"; Summary = "Focus the file tree."; Constructor = simple FocusTree }
          { Name = "editor"; Usage = "editor"; Summary = "Focus the editor."; Constructor = simple FocusEditor }
          { Name = "reload"; Usage = "reload"; Summary = "Reload the workspace tree."; Constructor = simple ReloadWorkspace }
          { Name = "next"; Usage = "next"; Summary = "Activate the next open buffer."; Constructor = simple NextBuffer }
          { Name = "prev"; Usage = "prev"; Summary = "Activate the previous open buffer."; Constructor = simple PreviousBuffer }
          { Name = "help"; Usage = "help"; Summary = "Show command help in the dock panel."; Constructor = simple Help } ]
    ...
    let parse (input: string) =
        ...
        if String.IsNullOrWhiteSpace trimmed then
            ...
            let name, argument =
                if firstSpace < 0 then trimmed, "" else trimmed[.. firstSpace - 1], trimmed[firstSpace + 1 ..]

            match specs |> List.tryFind (fun spec -> spec.Name = name.ToLowerInvariant()) with
            | Some spec -> spec.Constructor argument
            | None when specs |> List.exists (fun spec -> spec.Name.StartsWith(name, StringComparison.OrdinalIgnoreCase)) ->
    ...
    let completions context (input: string) =
        ...
        if String.IsNullOrWhiteSpace trimmed then
            specs
            |> List.map (fun spec -> { Label = spec.Name; ApplyText = spec.Name; Detail = spec.Summary; Kind = Command })
        else
            let firstSpace = trimmed.IndexOf ' '
            ...
            if firstSpace < 0 then
                ...
                | "writeas" ->
                    ...
                    |> List.truncate 8
                    |> List.map (fun filePath -> { Label = filePath; ApplyText = $"{commandName} {filePath}"; Detail = "workspace file"; Kind = PathItem })
                | _ ->
                    []
...
module Editor =
    ...
    let private notify notification model =
        { model with Notification = notification }
    ...
    let private updateActiveBuffer transform model =
        let viewportWidth = max 1 (model.Terminal.Width - (if model.Panels.SidebarVisible then model.Panels.SidebarWidth + 8 else 8))
        let viewportHeight = max 1 (model.Terminal.Height - model.Panels.DockHeight - 2)
        let updated = activeBufferState model |> transform |> Buffer.ensureViewport viewportHeight viewportWidth

        { model with
            Editors = { model.Editors with Buffers = model.Editors.Buffers |> Map.add updated.Id updated } }

    let private workspaceFiles (workspace: WorkspaceState) =
        let rec collect (node: FileNode) =
            if node.IsDirectory then node.Children |> List.collect collect else [ Path.GetRelativePath(workspace.RootPath, node.Path) ]

        workspace.Tree |> Option.map collect |> Option.defaultValue []

    let private refreshCommandBar model =
        let completions = Commands.completions { RootPath = model.Workspace.RootPath; Files = workspaceFiles model.Workspace } model.CommandBar.Text
        let selectedIndex = if completions.IsEmpty then 0 else min model.CommandBar.SelectedCompletion (completions.Length - 1)
    ...
    let private resolvePath (rootPath: string) (path: string) =
        if Path.IsPathRooted path then path else Path.GetFullPath(Path.Combine(rootPath, path))
    ...
    let private insertCommandText value model =
        ...
        { model with
            CommandBar = { model.CommandBar with Text = nextText; Cursor = cursor + value.Length; SelectedCompletion = 0 } }
        |> refreshCommandBar

    let private replaceCommandText value model =
        { model with
            CommandBar = { model.CommandBar with Text = value; Cursor = value.Length; SelectedCompletion = 0 } }
        |> refreshCommandBar
    ...
    let private saveActiveBuffer customPath model =
        ...
        | Some path ->
            model, [ SaveBuffer(buffer.Id, path, Buffer.text buffer |> fun text -> text.Replace("\n", buffer.Newline)) ]
        | None ->
            openCommandBar "writeas " model |> notify (Some(Notification.warning "Scratch buffers need a path.")), []
    ...
    let private switchBuffer offset model =
        ...
        if ids.IsEmpty then
            ...
            let currentIndex = ids |> List.tryFindIndex ((=) model.Editors.ActiveBufferId) |> Option.defaultValue 0
            let nextIndex =
                if offset > 0 then (currentIndex + 1) % ids.Length else (currentIndex - 1 + ids.Length) % ids.Length

            { model with Editors = { model.Editors with ActiveBufferId = ids[nextIndex] } }
    ...
    let private executeCommand command model =
        ...
        | Open path ->
            ...
            match model.Editors.Buffers |> Map.toList |> List.tryFind (fun (_, buffer) -> buffer.FilePath = Some absolutePath) with
            | Some (bufferId, _) ->
                { model with
                    Editors = { model.Editors with ActiveBufferId = bufferId }
                ...
                []
            | None ->
                { model with Focus = Editor }, [ LoadFile absolutePath ]
        | Write ->
            saveActiveBuffer None model
        | WriteAs path ->
            saveActiveBuffer (Some path) model
        | Quit ->
            { model with ShouldQuit = true }, []
        | ToggleSidebar ->
            { model with Panels = { model.Panels with SidebarVisible = not model.Panels.SidebarVisible } }, []
        | FocusTree ->
            { model with Focus = Sidebar }, []
        | FocusEditor ->
            { model with Focus = Editor }, []
        | ReloadWorkspace ->
            model, [ ScanWorkspace model.Workspace.RootPath ]
        | NextBuffer ->
            switchBuffer 1 model, []
        | PreviousBuffer ->
            switchBuffer -1 model, []
        | Help ->
            model |> notify (Some(Notification.info "Dock panel lists the current shortcuts and commands.")), []

    let private runSidebar key model =
        match key with
        | Up -> { model with Workspace = Workspace.moveSelection -1 model.Workspace }, []
        | Down -> { model with Workspace = Workspace.moveSelection 1 model.Workspace }, []
        | PageUp -> { model with Workspace = Workspace.moveSelection -10 model.Workspace }, []
        | PageDown -> { model with Workspace = Workspace.moveSelection 10 model.Workspace }, []
        | Home -> { model with Workspace = Workspace.moveHome model.Workspace }, []
        | End -> { model with Workspace = Workspace.moveEnd model.Workspace }, []
        | Left -> { model with Workspace = Workspace.collapseSelected model.Workspace }, []
        | Right -> { model with Workspace = Workspace.expandSelected model.Workspace }, []
        | Enter ->
    ...
    let private runEditor key model =
        ...
        | PageUp -> updateActiveBuffer (Buffer.movePageUp (max 1 (model.Terminal.Height - model.Panels.DockHeight - 2))) model, []
        | PageDown -> updateActiveBuffer (Buffer.movePageDown (max 1 (model.Terminal.Height - model.Panels.DockHeight - 2))) model, []
    ...
    let private runCommandBar key model =
        ...
        | Left -> { model with CommandBar = { model.CommandBar with Cursor = max 0 (model.CommandBar.Cursor - 1) } }, []
        | Right -> { model with CommandBar = { model.CommandBar with Cursor = min model.CommandBar.Text.Length (model.CommandBar.Cursor + 1) } }, []
        | Home -> { model with CommandBar = { model.CommandBar with Cursor = 0 } }, []
        | End -> { model with CommandBar = { model.CommandBar with Cursor = model.CommandBar.Text.Length } }, []
        ...
        | Tab when not model.CommandBar.Completions.IsEmpty ->
            { model with
                CommandBar = { model.CommandBar with SelectedCompletion = (model.CommandBar.SelectedCompletion + 1) % model.CommandBar.Completions.Length } },
            []
        | ShiftTab when not model.CommandBar.Completions.IsEmpty ->
            let count = model.CommandBar.Completions.Length
            { model with CommandBar = { model.CommandBar with SelectedCompletion = (model.CommandBar.SelectedCompletion + count - 1) % count } }, []
        | Up when not model.CommandBar.History.IsEmpty ->
            ...
            replaceCommandText model.CommandBar.History[index] { model with CommandBar = { model.CommandBar with HistoryIndex = Some index } }, []
        ...
        | Enter ->
            ...
            | Pending _ when not model.CommandBar.Completions.IsEmpty ->
                ...
                | None -> model, []
            | Pending message ->
                notify (Some(Notification.warning message)) model, []
            | Invalid message ->
                notify (Some(Notification.error message)) model, []
            | Empty ->
                closeCommandBar model, []
        | _ ->
            model, []

    let init rootPath size =
    ...
    let update msg model =
        match msg with
        | Resize size ->
            { model with Terminal = size } |> updateActiveBuffer id, []
        ...
        | FileOpened(path, result) ->
            ...
            | Result.Ok contents ->
                ...
                let buffer = Buffer.fromText model.Editors.NextBufferId (Some path) (Path.GetFileName path) normalized newline
        ...
        | BufferSaved(bufferId, path, result) ->
            ...
            | Result.Ok () ->
                { model with
                    Editors =
                        { model.Editors with
                            Buffers = model.Editors.Buffers |> Map.add bufferId (Buffer.markSaved path model.Editors.Buffers[bufferId]) }
                    Notification = Some(Notification.info $"Saved {Path.GetFileName path}") },
                []
            | Result.Error message ->
                notify (Some(Notification.error $"Failed to save {path}: {message}")) model, []
        | KeyPressed key ->
            match key with
            | Ctrl 'q' -> { model with ShouldQuit = true; Notification = None }, []
            | Ctrl 'p' -> openCommandBar "" { model with Notification = None }, []
            | Ctrl 'b' -> { model with Focus = Sidebar; Notification = None }, []
            | Ctrl 'e' -> { model with Focus = Editor; Notification = None }, []
    ...
    let statusLine model =
        let buffer = activeBufferState model
        let focusText = match model.Focus with | Sidebar -> "TREE" | Editor -> "EDIT" | CommandBar -> "CMD"
        let dirty = if buffer.Dirty then " [+]" else ""
        let note = model.Notification |> Option.map _.Message |> Option.defaultValue "Ctrl+P commands"
    ...
    let dockPanel model =
        if model.CommandBar.Active && not model.CommandBar.Completions.IsEmpty then
            ...
            let buffer = activeBufferState model
            DockInfo("Editor", [ $"Buffer: {buffer.Name}"; $"Open buffers: {model.Editors.Buffers.Count}"; "Ctrl+B tree, Ctrl+E editor"; "Ctrl+P commands, Ctrl+S save"; "Tab indent, Shift+Tab unindent" ])
...
module Style =
    ...
    let withColors foreground background =
        { defaultStyle with Foreground = foreground; Background = background }
...
module Screen =
    ...
    let writeText x y style maxWidth (text: string) screen =
        if y >= 0 && y < screen.Height && maxWidth > 0 then
            text |> Seq.truncate maxWidth |> Seq.iteri (fun index glyph -> setCell (x + index) y style glyph screen)
    ...
    let withCursor cursor (screen: Screen) =
        { screen with Cursor = Some cursor }
...
module Renderer =
    ...
    let private sgr style =
        let parts =
            [ "0"
              if style.Bold then "1"
              if style.Inverted then "7"
    ...
    let enter renderer = renderer.Writer.Write("\u001b[?1049h\u001b[?25l\u001b[2J")
    let leave renderer = renderer.Writer.Write("\u001b[0m\u001b[?25h\u001b[?1049l")
    ...
    let render renderer screen =
        ...
        | Some cursor when cursor.Visible ->
            builder.Append($"\u001b[?25h\u001b[{cursor.Top + 1};{cursor.Left + 1}H") |> ignore
        | _ ->
            builder.Append("\u001b[?25l") |> ignore
...
[<RequireQualifiedAccess>]
module Input =
    let private hasModifier modifier (keyInfo: ConsoleKeyInfo) =
        keyInfo.Modifiers.HasFlag modifier
...
module Layout =
    ...
    let private accent = Style.withColors (Indexed 81) Default
    let private status = Style.withColors (Indexed 15) (Indexed 24)
    let private commandBar = Style.withColors (Indexed 230) (Indexed 237)
    let private selected = { commandBar with Background = Indexed 31; Bold = true }
    let private lineNumber = Style.withColors (Indexed 241) Default
    let private currentLineNumber = Style.withColors (Indexed 153) Default
    ...
    let private crop start width (text: string) =
        if width <= 0 || start >= text.Length then "" else text.Substring(start, min width (text.Length - start))
    ...
    let private renderSidebar width height screen model =
        ...
        let selectedIndex = entries |> List.tryFindIndex _.IsSelected |> Option.defaultValue 0
        let startIndex = max 0 (min (max 0 (entries.Length - height)) (selectedIndex - (height / 2)))
    ...
    let private renderEditor x width height screen model =
        ...
        let digits = max 3 (string rows.Length).Length
        let gutterWidth = digits + 2
        ...
        for row in 0 .. height - 1 do
            ...
            if lineIndex < rows.Length then
                ...
                let textStyle = if activeLine && model.Focus = Editor then selected else surface
                let lineNumberText = $"{lineIndex + 1}".PadLeft(digits) + " "
                Screen.writeText x row (if activeLine then currentLineNumber else lineNumber) gutterWidth lineNumberText screen
                Screen.writeText (x + gutterWidth) row textStyle contentWidth (pad contentWidth (crop buffer.ViewportLeft contentWidth rows[lineIndex])) screen
            else
        ...
        if model.Focus = Editor then
            ...
            if cursorX >= x + gutterWidth && cursorX < x + width && cursorY >= 0 && cursorY < height then
                Screen.withCursor { Left = cursorX; Top = cursorY; Visible = true } screen
            else
                screen
        else
            screen
    ...
    let render model =
        ...
        let sidebarWidth = if model.Panels.SidebarVisible && width >= 40 then min model.Panels.SidebarWidth (max 18 (width / 3)) else 0
        ...
        | DockInfo(title, lines) ->
            ...
            |> List.iteri (fun index lineText -> Screen.writeText 1 (dockY + index + 1) chrome (max 0 (width - 2)) (pad (max 0 (width - 2)) lineText) current)
        | DockCompletions(title, items, selectedIndex) ->
            ...
            |> List.iteri (fun index item ->
                ...
                Screen.writeText 1 (dockY + index + 1) style (max 0 (width - 2)) (pad (max 0 (width - 2)) $"{prefix}{item.Label}  {item.Detail}") current)

        Screen.fillRect 0 commandY width 1 commandBar ' ' current
        let lineText = if model.CommandBar.Active then ":" + model.CommandBar.Text else " Ctrl+P commands  Ctrl+B tree  Ctrl+S save  Ctrl+Q quit "
        Screen.writeText 0 commandY commandBar width (pad width lineText) current

        if model.CommandBar.Active then
            current <- Screen.withCursor { Left = min (width - 1) (1 + model.CommandBar.Cursor); Top = commandY; Visible = true } current
...
module Runtime =
    ...
    let private runEffect effect =
        match effect with
        | ScanWorkspace rootPath ->
            try WorkspaceLoaded(Result.Ok(scanNode rootPath)) with ex -> WorkspaceLoaded(Result.Error ex.Message)
        | LoadFile path ->
            try FileOpened(path, Result.Ok(File.ReadAllText path)) with ex -> FileOpened(path, Result.Error ex.Message)
    ...
    let rec private dispatch model msg =
        ...
        effects |> List.fold (fun state effect -> dispatch state (runEffect effect)) nextModel
    ...
    let run rootPath =
        ...
        let initialModel, startupEffects = Editor.init rootPath (consoleSize ())
        let mutable model = startupEffects |> List.fold (fun state effect -> dispatch state (runEffect effect)) initialModel
